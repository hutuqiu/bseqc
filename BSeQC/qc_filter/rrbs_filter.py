...
def get_filted_info(strand, seq, score, CIGAR, site_meth_list, single_on, reverse_strand, filter_MspI_end_repaired_bp,
                    pos_list):
    '''
    ...
    if strand in original_read:
        ...
        if strand in reverse_strand:
            if single_on:
                pos_list[0] += readlen - len(seq)
            else:
                pos_list[1] += readlen - len(seq)
    else:
        if not single_on:
